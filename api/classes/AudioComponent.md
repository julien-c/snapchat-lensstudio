...
Used to play audio in a Lens. You can assign an
[AudioTrackAsset](/api/classes/AudioTrackAsset) to play through script
or through the AudioComponent’s inspector in Lens Studio. See the
[Playing Audio](/guides/audio) guide for more information.
...
### `play(Number loops)` : `void`

Plays the current sound `loops` number of times. If `loops` is -1, the
sound will repeat forever.
...
### `setOnFinish(function eventCallback)` : `void`

Sets the callback function to be called whenever this sound stops
playing.

-----

### `stop(Boolean fade)` : `void`
...
### `audioTrack` : [`AudioTrackAsset`](/api/classes/AudioTrackAsset)
...
Length (in seconds) of a volume fade in applied to the beginning of
sound playback.
...
Length (in seconds) of a volume fade out applied to the end of sound
playback.
