...
Controls the position, rotation, and scale of a
[SceneObject](/api/classes/SceneObject). Every SceneObject automatically
has a Transform attached.

## Methods

-----

### `getInvertedWorldTransform()` : [`mat4`](/api/classes/mat4)

Returns the Transform’s world-to-local transformation matrix.

-----

### `getLocalPosition()` : [`vec3`](/api/classes/vec3)

Returns the Transform’s position relative to its parent.

-----

### `getLocalRotation()` : [`quat`](/api/classes/quat)

Returns the Transform’s rotation relative to its parent.

-----

### `getLocalScale()` : [`vec3`](/api/classes/vec3)
