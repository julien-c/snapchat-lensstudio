...
Inherits from [SpriteVisual](/api/classes/SpriteVisual)

## Description

Displays text in the scene. This is now deprecated in favor of
[Text](/api/classes/Text). See the [Text guide](/guides/2d/text) for
more information.
...
### `measureText(String text)` : [`vec2`](/api/classes/vec2)

Returns the potential width and height of the Label if it were to
...
### `fontAsset` : [`Font`](/api/classes/Font)

The font used to display text.

-----

### `outlineColor` : [`vec4`](/api/classes/vec4)
...
### `shadowColor` : [`vec4`](/api/classes/vec4)

The color used for dropshadow.

-----

### `shadowOffset` : [`vec2`](/api/classes/vec2)
...
### `textColor` : [`vec4`](/api/classes/vec4)
...
### `getMeshSize()` : [`vec2`](/api/classes/vec2)

Returns the width and height of the mesh the `SpriteVisual` is applied
to.
...
Adds a Material to the MeshVisual.

-----

### `clearMaterials()` : `void`

Clears all Materials from the MeshVisual.

-----

### `getMaterial(Number index)` : [`Material`](/api/classes/Material)
...
### `setBlendShape(BlendShapesVisual value)` : `void`

Sets the BlendShape for this MeshVisual.

-----

### `setSkin(Skin value)` : `void`

Sets the [Skin](/api/classes/Skin) used by this MeshVisual.

-----

### `snap(Camera camera)` : `void`

Projects screen positions from `camera`’s view onto the mesh’s UVs. If
the MeshVisual’s material uses the same texture as the camera input, the
MeshVisual will look identical to the part of the screen it covers.
...
### `setRenderOrder(Number value)` : `void`
...
### `pivot` : [`vec2`](/api/classes/vec2)

The location of the sprite’s pivot point relative to its boundaries. The
pivot’s `x` value is a relative horizontal position, -1 being the
sprite’s left border and 1 being the sprite’s right border. Similarly,
the `y` value is a relative vertical position, -1 being the sprite’s
bottom border and 1 being the sprite’s top border.

-----

### `horizontalAlignment` : `Number`

When a [ScreenTransform](/api/classes/ScreenTransform) is attached to
the same SceneObject, this controls how the mesh will be positioned
horizontally depending on `stretchMode`.
...
### `mainMaterial` : [`Material`](/api/classes/Material)

Returns the first Material.

-----

### `mainPass` : [`Pass`](/api/classes/Pass)

Returns the `mainPass` of the `mainMaterial`.

-----

### `mesh` : [`RenderMesh`](/api/classes/RenderMesh)

The RenderMesh asset this MeshVisual will render.
...
### `stretchMode` : [`StretchMode`](/api/classes/StretchMode)

When a [ScreenTransform](/api/classes/ScreenTransform) is attached to
the same SceneObject, this controls how the mesh will be stretched
relative to the ScreenTransform’s boundaries.
...
When a [ScreenTransform](/api/classes/ScreenTransform) is attached to
the same SceneObject, this controls how the mesh will be positioned
vertically depending on `stretchMode`.
