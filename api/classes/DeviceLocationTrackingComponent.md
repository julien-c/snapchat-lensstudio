...
Used to track a landmarker in the camera. Moves the SceneObject’s
transform to match the detected landmarker scene. See the [Landmarker
guide](/templates/landmarker) for more information.
...
### `locationProximityStatus` : [`LocationProximityStatus`](/api/classes/LocationProximityStatus)

Returns the user’s current LocationProximityStatus. Useful for telling
if a user is close enough to the location to track it.

-----

### `onLocationDataDownloaded` : `function`

A function that gets called when location data is downloaded.

-----

### `onLocationDataDownloadFailed` : `function`
...
### `onLocationLost` : `function`

A function that gets called when location is lost. Note this will also
happen when the user flips the camera.
