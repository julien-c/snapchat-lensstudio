...
Allows data to be stored and retrieved between Lens sessions. In other
words, data can be saved on device and loaded back in the next time the
Lens is opened. Can be accessed with `global.persistentStorageSystem`.

See the [Persistent Storage guide](/guides/scripting/persistent-storage)
...
### `store` : [`GeneralDataStore`](/api/classes/GeneralDataStore)
