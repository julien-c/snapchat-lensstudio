...
Handles input information from user touch input via the
[TouchComponent](/api/classes/TouchComponent) to control Scale,
...
Repositions the object to be within the bounds of `minDistance`,
`maxDistance`.

-----

### `enableManipulateType(ManipulateType type, Boolean enable)` : `void`

Enables or disables the specified ManipulateType for this
ManipulateComponent.

-----

### `intersectManipulateFrame(vec2 screenSpacePoint)` : [`ManipulateFrameIntersectResult`](/api/classes/ManipulateFrameIntersectResult)

Checks for an intersection point between the manipulation plane and a
line extending from the camera through the specified screen space point.
The screen point is passed in as (x, y) with both values ranging from
(\[0-1\], \[0-1\]), (0,0) being left-top and (1,1) being right-bottom.
...
### `isManipulateTypeEnabled(ManipulateType type)` : `Boolean`

Returns whether the specified ManipulateType is enabled for this
ManipulateComponent.
...
Changes swivel behavior based on the object’s height relative to the
camera.
...
Multiplier for swivel rotation speed. For example, a value of 0.5 will
cut rotation speed in half, and a value of 2.0 will double rotation
speed.
