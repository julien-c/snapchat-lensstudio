...
Used in [Camera’s](/api/classes/Camera) `depthBufferMode` property. Each
mode is suited for handling objects at a certain distance range. For
more information on depth modes, see the [Camera and
Layers](/guides/general/camera) guide.

| Enum Value                           | Description                                                                                                |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| `Camera.DepthBufferMode.Regular`     | Gives higher depth precision on nearby objects, so is better suited for scenes near to the camera.         |
| `Camera.DepthBufferMode.Logarithmic` | Gives higher depth precision on far away objects, so is better suited for scenes far away from the camera. |
