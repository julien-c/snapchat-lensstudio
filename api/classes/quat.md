...
### `new quat(Number w, Number x, Number y, Number z)` : [`quat`](/api/classes/quat)
...
### `getAxis()` : [`vec3`](/api/classes/vec3)

Returns the rotation axis of the quat.

-----

### `invert()` : [`quat`](/api/classes/quat)

Returns an inverted version of the quat.

-----

### `multiply(quat b)` : [`quat`](/api/classes/quat)
...
### `toEulerAngles()` : [`vec3`](/api/classes/vec3)

Returns an euler angle representation of the quat, in radians.

-----

### `static angleAxis(Number angle, vec3 axis)` : [`quat`](/api/classes/quat)

Returns a new quat with angle `angle` and axis `axis`.

-----

### `static angleBetween(quat a, quat b)` : `Number`

Returns the angle between `a` and `b`.

-----

### `static fromEulerAngles(Number x, Number y, Number z)` : [`quat`](/api/classes/quat)

Returns a new quat using the euler angles `x`, `y`, `z` (in radians).

-----

### `static fromEulerVec(vec3 eulerVec)` : [`quat`](/api/classes/quat)

Returns a new quat using the euler angle `eulerVec` (in radians).

-----

### `static lerp(quat a, quat b, Number t)` : [`quat`](/api/classes/quat)

Returns a new quat linearly interpolated between `a` and `b`.

-----

### `static lookAt(vec3 forwardIn, vec3 upIn)` : [`quat`](/api/classes/quat)

Returns a new quat with a forward vector `forwardIn` and up vector
`upIn`.

-----

### `static quatIdentity()` : [`quat`](/api/classes/quat)

Returns the identity quaternion.

-----

### `static rotationFromTo(vec3 from, vec3 to)` : [`quat`](/api/classes/quat)

Returns a rotation quat between direction vectors `from` and `to`.

-----

### `static slerp(quat a, quat b, Number t)` : [`quat`](/api/classes/quat)

Returns a new quat spherically linearly interpolated between `a` and
`b`.
