...
An asset that describes how visual objects should appear. Each Material
is a collection of [Passes](/api/classes/Pass) which define the actual
rendering passes. Materials are used by
[MeshVisuals](/api/classes/MeshVisual) for drawing meshes in the scene.

## Methods

-----

### `clone()` : [`Material`](/api/classes/Material)

Returns a copy of the Material.

-----

### `getPass(Number index)` : [`Pass`](/api/classes/Pass)

Returns the [Pass](/api/classes/Pass) of the Material at index `index`.

-----

### `getPassCount()` : `Number`

Returns the number of [Passes](/api/classes/Pass) for the Material.
