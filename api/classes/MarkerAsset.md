...
Defines a marker to use for Marker Tracking with
[MarkerTrackingComponent](/api/classes/MarkerTrackingComponent). For
more information, see the [Marker
Tracking](/guides/general/marker-tracking) guide.
...
Returns the aspect ratio (width / height) of the texture used by the
marker asset.
...
The height of the marker asset in real-life centimeters. Used to provide
accurate transformation. A
[MarkerTrackingComponent](/api/classes/MarkerTrackingComponent) using
this MarkerAsset will be scaled so that one unit in the SceneObject’s
local space is equal to one centimeter in real life.
