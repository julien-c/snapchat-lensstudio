...
Options for stretching a mesh to fit a
[ScreenTransform’s](/api/classes/ScreenTransform) bounding box. Used
in [MeshVisual’s](/api/classes/MeshVisual) `stretchMode` property, as
long as the SceneObject has a
[ScreenTransform](/api/classes/ScreenTransform) attached.

See the [Image guide](/guides/2d/image/) for more information about
stretch modes.

| Enum Value               | Description                                                                                                                 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| `StretchMode.Fit`        | Scale uniformly so that both width and height fit within the bounds.                                                        |
| `StretchMode.Fill`       | Scale uniformly so that both width and height meet or exceed the bounds.                                                    |
| `StretchMode.Stretch`    | Scale non-uniformly to match the exact width and height of the bounds.                                                      |
| `StretchMode.FitHeight`  | Scale uniformly to match the same height as the bounds.                                                                     |
| `StretchMode.FitWidth`   | Scale uniformly to match the same width as the bounds.                                                                      |
| `StretchMode.FillAndCut` | Same as `Fill`, but when used with the [Image component](/api/classes/Image) any area outside of the bounds is cropped out. |
