...
Used by [AnimationMixer](/api/classes/AnimationMixer) to animate a
single object in the hierarchy. These are automatically added to
SceneObjects when importing animated FBX files. See also: [Playing 3D
...
### `getAnimationLayerByName(String layerName)` : [`AnimationLayer`](/api/classes/AnimationLayer)

Returns the AnimationLayer under the name `layerName`.

-----

### `removeAnimationLayerByName(String layerName)` : `void`

Removes the AnimationLayer under the name `layerName`.

-----

### `setAnimationLayerByName(String layerName, AnimationLayer animationLayerPtr)` : `void`
