...
See the [Pin To Mesh](/guides/3d/pin-to-mesh) guide for more
information.
...
### `offsetPosition` : [`vec3`](/api/classes/vec3)

The position offset to apply.

-----

### `offsetRotation` : [`vec3`](/api/classes/vec3)
...
### `orientation` : [`PinToMeshComponent.Orientation`](/api/classes/PinToMeshComponent_Orientation)

The orientation type to use.

-----

### `pinUV` : [`vec2`](/api/classes/vec2)
...
The preferred triangle index to attach to when multiple triangles
contain the desired UV coordinate.

-----

### `target` : [`MeshVisual`](/api/classes/MeshVisual)
...
If enabled, interpolated vertex normals will be used when calculating
