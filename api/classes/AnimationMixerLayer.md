...
Controls animation playback for a single animation layer. See also:
[AnimationMixer](/api/classes/AnimationMixer).
...
### `clone(String newName)` : [`AnimationMixerLayer`](/api/classes/AnimationMixerLayer)

Returns a copy of this AnimationMixerLayer, with the name changed to
`newName`.
...
### `start(Number offsetArg, Number cycles)` : `void`

Starts playing the animation with an offset of `offsetArg` seconds. The
animation will play `cycles` times, or loop forever if `cycles` is -1.

-----

### `startWithCallback(Number offsetArg, Number cycles, function eventCallback)` : `void`

Starts the animation with an offset of `offsetArg` seconds. The
animation will play `cycles` times, or loop forever if `cycles` is -1.
...
The number of times this animation will play. If -1, the animation will
loop forever.
...
The starting point for this animation clip. If `rangeType` is set to
`Time`, this is the point to start at in seconds. If `rangeType` is set
to `Frames`, this is the frame number to start at.
...
### `postInfinity` : [`AnimationClip.PostInfinity`](/api/classes/AnimationClip_PostInfinity)

Defines the animation’s looping behavior. If set to
`AnimationClip.PostInfinity.Cycle`, the animation will restart from the
beginning each time it loops. If set to
`AnimationClip.PostInfinity.Oscillate`, the animation will switch
between normal and reverse playback each time it loops. This is set to
`Cycle` by default.

-----

### `rangeType` : [`AnimationClip.RangeType`](/api/classes/AnimationClip_RangeType)

The range type used for defining the animation clip. If set to
`AnimationClip.RangeType.Time`, `to` and `from` represent times in
seconds. If set to `AnimationClip.RangeType.Frames`, `to` and `from`
...
A multiplying value for the speed of this animation. For example, a
value of 2.0 will double animation speed, while a value of 0.5 will cut
the speed in half.
...
The ending point for this animation clip. If `rangeType` is set to
`Time`, this is the point to end at in seconds. If `rangeType` is set to
`Frames`, this is the frame number to end at.

-----

### `weight` : `Number`

The weight of this animation layer. Range is from \[0-1\], 0 being no
animation strength and 1 being full animation strength.
