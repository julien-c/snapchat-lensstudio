...
Used in [Text’s](/api/classes/Text) `dropShadowSettings` property.
...
### `fill` : [`TextFill`](/api/classes/TextFill)

Settings for how the inside of the dropshadow is drawn.

-----

### `offset` : [`vec2`](/api/classes/vec2)

An (x, y) offset controlling where the dropshadow is drawn relative to
the Text.
