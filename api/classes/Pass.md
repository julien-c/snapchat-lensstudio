...
Controls how a mesh will get rendered. Each Pass acts as an interface
for the shader it’s associated with. Any properties on a Pass’s shader
will automatically become properties on that Pass. For example, if the
shader defines a variable named `baseColor`, a script would be able to
access that property as `Pass.baseColor`.
...
### `colorMask` : [`vec4b`](/api/classes/vec4b)

Controls the masking of color channels with a vec4b representing each
channel with a boolean.

-----

### `cullMode` : [`CullMode`](/api/classes/CullMode)
...
### `polygonOffset` : [`vec2`](/api/classes/vec2)
