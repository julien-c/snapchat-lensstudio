...
Controls an animated texture resource. Can be accessed from
[Texture.control](/api/classes/Texture#control-textureprovider) on an
animated texture. See also: [2D Animation
Guide](/guides/2d/2d-animation).
...
### `pauseAtFrame(Number frameIndex)` : `void`

Pauses the animation at frame `frameIndex`.

-----

### `play(Number loops, Number offset)` : `void`

Plays the animation `loops` times, starting with an offset of `offset`
seconds.

-----

### `playFromFrame(Number frameIndex, Number loops)` : `void`
...
Sets the callback function to be called whenever the animation stops
playing.
...
If enabled, the animation will alternate between normal and reverse each
time it loops.
...
### `track` : [`IntStepAnimationTrackKeyFramed`](/api/classes/IntStepAnimationTrackKeyFramed)
