...
Every frame, LookAtComponent rotates its SceneObject to face towards a
target SceneObject.
...
### `aimVectors` : [`LookAtComponent.AimVectors`](/api/classes/LookAtComponent_AimVectors)

The “aim” and “up” vectors used when determining rotation.
LookAtComponent will try to point the `Aim` axis of the SceneObject
towards the target, while keeping the `Up` axis of the SceneObject
pointing towards `worldUpVector`.

-----

### `target` : [`SceneObject`](/api/classes/SceneObject)

The SceneObject this LookAtComponent targets.

-----

### `worldUpVector` : [`LookAtComponent.WorldUpVector`](/api/classes/LookAtComponent_WorldUpVector)
