...
Inherits from [SerializableWithUID](/api/classes/SerializableWithUID)

## Description

An object in the scene hierarchy, containing a
[Transform](/api/classes/Transform) and possibly
[Components](/api/classes/Component). A script can access the
SceneObject holding it through the method `script.getSceneObject()`.

## Methods

-----

### `copyComponent(Component component)` : [`Component`](/api/classes/Component)

Copies `component` and adds it to the SceneObject, then returns it.

-----

### `copySceneObject(SceneObject sceneObject)` : [`SceneObject`](/api/classes/SceneObject)

Creates a shallow copy of the passed in `sceneObject` (not including its
hierarchy), and parents it to this SceneObject.

-----

### `copyWholeHierarchy(SceneObject sceneObject)` : [`SceneObject`](/api/classes/SceneObject)

Creates a deep copy of the passed in `sceneObject` (including its
hierarchy), and parents it to this SceneObject.

-----

### `createComponent(String typeName)` : [`Component`](/api/classes/Component)
...
### `getAllComponents()` : [`Component`](/api/classes/Component)\[\]

Returns a list of all components attached to the SceneObject.

-----

### `getChild(Number index)` : [`SceneObject`](/api/classes/SceneObject)
...
### `getComponentByIndex(String componentType, Number index)` : [`Component`](/api/classes/Component)

Returns the attached component of type `componentType` at index `index`.
If `componentType` is an empty string, all component types are
considered.

-----

### `getComponentCount(String componentType)` : `Number`

Returns the number of components of type `componentType` attached to the
SceneObject. If `componentType` is an empty string, the total number of
components attached is returned.

-----

### `getFirstComponent(String componentType)` : [`Component`](/api/classes/Component)

Returns the first attached component of type `componentType`. If
`componentType` is an empty string, the first component of any type is
returned.

-----

### `getParent()` : [`SceneObject`](/api/classes/SceneObject)

Returns the SceneObject’s parent in the hierarchy, or null if there
isn’t one.
...
### `setParent(SceneObject newParent)` : `void`
...
Whether the SceneObject, including its components and children, is
enabled or disabled.

-----

### `layer` : [`LayerSet`](/api/classes/LayerSet)

Gets or sets the LayerSet of layers this SceneObject belongs to. This is
used to determine which [Cameras](/api/classes/Camera) will render the
SceneObject.
...
## Examples
    ...
    // Look for a MeshVisual on this SceneObject
    var sceneObj = script.getSceneObject();
    if(sceneObj.getComponentCount("Component.MeshVisual") > 0)
    {
